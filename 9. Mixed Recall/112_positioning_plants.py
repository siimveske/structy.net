...
def positioning_plants(costs):
    return explore(costs, 0, None, {})


def explore(costs, row, prev, memo):
    if row == len(costs):
        return 0
    ...
    results = []
    for idx, cost in enumerate(costs[row]):
        if idx == prev:
            continue
        results.append(cost + explore(costs, row + 1, idx, memo))
